package com.xingheyuzhuan.shiguangschedule.widget.wide_double_days
...
object WideDoubleDaysNativeRenderer {

    fun render(context: Context, snapshot: WidgetSnapshot): RemoteViews {
        val rv = RemoteViews(context.packageName, R.layout.widget_wide_double_days_native)
        resetWidgetState(rv)
        ...
        renderColumn(
            ...
            today, remainingToday.take(3), remainingToday.size,
    ...
    private fun renderColumn(
        context: Context, rootRv: RemoteViews, containerId: Int, dateId: Int, footerId: Int,
        emptyContainerId: Int, date: LocalDate, displayCourses: List<WidgetCourseProto>,
        totalCount: Int, footerResId: Int, snapshot: WidgetSnapshot
    ) {
        val prefix = if (date == LocalDate.now()) context.getString(R.string.widget_title_today)
        else context.getString(R.string.widget_title_tomorrow)
